...
def measure_layer(layer, x):
    ...
    ### ops_nonlinearity
    elif type_name in ['ReLU', "InputQuantizer", "activation_quantize_fn", "activation_quantize_fn2", "Binarize01Act"]:
        delta_ops = x.numel()
        delta_params = get_layer_param(layer)
    ...
    ### ops_nothing
        ...
        delta_params = get_layer_param(layer)
    elif type_name in ['ZeroPad2d', "weight_quantize_fn"]:
        pass
    ### unknown layer type
    else:
...
def measure_model(model, C, H, W, device="cpu"):
    global count_ops, count_params
    count_ops = 0
    count_params = 0
