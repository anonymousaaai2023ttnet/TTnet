...
class model_general(SeqBinModelHelper, nn.Module, ModelHelper):
    ...
    def make_small_network(
        ...
        if len(args.filters) == 3:
            ...
            act(),  # 2
            nn.Conv2d(args.nchannel, args.filters[0] * args.amplifications[0], args.kernelsizes[0],
                      stride=args.strides[0], padding=0,
                      groups=args.groups[0]),
            nn.BatchNorm2d(args.filters[0] * args.amplifications[0]),
            nn.ReLU(),
            nn.Conv2d(args.filters[0] * args.amplifications[0], args.filters[0], 1, stride=1, padding=0,
                      groups=args.groups[0]),
            nn.BatchNorm2d(args.filters[0]),
            act(),
            nn.Conv2d(args.filters[0], args.filters[1] * args.amplifications[1], args.kernelsizes[1],
                      stride=args.strides[1], padding=0,
                      groups=args.groups[1]),
            nn.BatchNorm2d(args.filters[1] * args.amplifications[1]),
            nn.ReLU(),
            nn.Conv2d(args.filters[1] * args.amplifications[1], args.filters[1], 1, stride=1, padding=0,
                      groups=args.groups[1]),
            nn.BatchNorm2d(args.filters[1]),
            act(),
            nn.Conv2d(args.filters[1], args.filters[2] * args.amplifications[2], args.kernelsizes[2],
                      stride=args.strides[2], padding=0,
                      groups=args.groups[2]),
            nn.BatchNorm2d(args.filters[2] * args.amplifications[2]),
            nn.ReLU(),
            nn.Conv2d(args.filters[2] * args.amplifications[2], args.filters[2], 1, stride=1, padding=0,
                      groups=args.groups[2]),
            nn.BatchNorm2d(args.filters[2]),
            act(),
            Flatten(),
            lin(wb, args.fc, nclass),
            setattr_inplace(
            ...
            nn.BatchNorm2d(args.filters[2]),
            act(),
            nn.Conv2d(args.filters[2], args.filters[3] * args.amplifications[3], args.kernelsizes[3],
                      stride=args.strides[3], padding=0,
                      groups=args.groups[3]),
            nn.BatchNorm2d(args.filters[3] * args.amplifications[3]),
            nn.ReLU(),
            nn.Conv2d(args.filters[3] * args.amplifications[3], args.filters[3], 1, stride=1, padding=0,
                      groups=args.groups[3]),
            nn.BatchNorm2d(args.filters[3]),
            act(),
            Flatten(),
            lin(wb, args.fc, nclass),
            setattr_inplace(
            ...
            nn.BatchNorm2d(args.nchannel),  # 1
            act(), #2
            nn.Conv2d(args.nchannel, args.nchannel* args.amplifications[0], args.kernelsizes[0], stride=args.strides[0], padding=0,
                      groups=args.groups[0]),
            nn.BatchNorm2d(args.nchannel * args.amplifications[0]),
            nn.ReLU(),
            nn.Conv2d(args.nchannel * args.amplifications[0], args.filters[0], 1, stride=1, padding=0,
                      groups=args.groups[0]),
            nn.BatchNorm2d(args.filters[0]),
            act(),
            nn.Conv2d(args.filters[0], args.filters[1]* args.amplifications[1], args.kernelsizes[1], stride=args.strides[1], padding=0,
                      groups=args.groups[1]),
            nn.BatchNorm2d(args.filters[1]* args.amplifications[1]),
            nn.ReLU(),
            nn.Conv2d(args.filters[1]* args.amplifications[1], args.filters[1], 1, stride=1, padding=0, groups=args.groups[1]),
            nn.BatchNorm2d(args.filters[1]),
            act(),
            Flatten(),
            lin(wb, args.fc, nclass),
...
class model_sans_ampli(SeqBinModelHelper, nn.Module, ModelHelper):
    """see https://github.com/locuslab/convex_adversarial/blob/b3f532865cc02aeb6c020b449c2b1f0c13c7f236/examples/problems.py#L92"""
    ...
    def make_small_network(
        ...
        if len(args.filters) == 3:
            ...
            nn.BatchNorm2d(args.nchannel),  # 1
            act(), #2
            nn.Conv2d(args.nchannel, args.filters[0], args.kernelsizes[0], stride=args.strides[0], padding=0,
                      groups=args.groups[0]),
            nn.BatchNorm2d(args.filters[0]),
            act(),
            nn.Conv2d(args.filters[0], args.filters[1], args.kernelsizes[1], stride=args.strides[1], padding=0,
                      groups=args.groups[1]),
            nn.BatchNorm2d(args.filters[1]),
            act(),
            Flatten(),
...
class model_general_activbin3(SeqBinModelHelper, nn.Module, ModelHelper):
    """see https://github.com/locuslab/convex_adversarial/blob/b3f532865cc02aeb6c020b449c2b1f0c13c7f236/examples/problems.py#L92"""
...
class model_general_realLR(SeqBinModelHelper, nn.Module, ModelHelper):
    """see https://github.com/locuslab/convex_adversarial/blob/b3f532865cc02aeb6c020b449c2b1f0c13c7f236/examples/problems.py#L92"""
...
class model_general_all(SeqBinModelHelper, nn.Module, ModelHelper):
    """see https://github.com/locuslab/convex_adversarial/blob/b3f532865cc02aeb6c020b449c2b1f0c13c7f236/examples/problems.py#L92"""
...
class model_general_qwant3(SeqBinModelHelper, nn.Module, ModelHelper):
    """see https://github.com/locuslab/convex_adversarial/blob/b3f532865cc02aeb6c020b449c2b1f0c13c7f236/examples/problems.py#L92"""
