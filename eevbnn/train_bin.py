from src.utils.config import transform_input_filters
    ...
    def __init__(self, model):
        ...
        for i in model.features:
            if isinstance(i, (net_bin.BinConv2d, net_bin.BinLinear)):
                ...
                mask = i._parameters.get('weight_mask')
                if mask is not None:
                    weights.append(mask)
...
class TrainingLoop:
    ...
    def train_epoch(self, epoch):
        ...
        spar_stat = self._net.get_sparsity_stat()
        if (spar_stat) is not None:
            ...
            logmsg(f'sparsity@{epoch}: {spar_parts} ; '
                   f'{spar_nz}/{spar_tot+0.0001}={spar_nz/(spar_tot+0.0001)*100:.2f}%')
    ...
    def _compute_losses(self, inputs, inputs_adv, labels, ep, ms):
        ...
        #loss_W = 0
        #coefinter = self._net.features[15].pre_fc_spatial2
        #for ixxx in range(10):
        #    for jxxx in range(ixxx, 10):
        #        if ixxx!=jxxx:
                    #print(self._net.features[15].fc_all[ixxx].weight_bin.squeeze(0))
                    #print(self._net.features[15].fc_all[ixxx].weight_bin.squeeze(0).shape)
                    #for batchici2 in range(self._net.features[15].featuresf.shape[0]):
                    #print(self._net.features[15].featuresf.shape)
        #            M_inter = torch.mm(self._net.features[15].featuresf[:, coefinter * ixxx:coefinter * ixxx + coefinter],
        #                     self._net.features[15].featuresf[:, coefinter * ixxx:coefinter * ixxx + coefinter].transpose(0,1))
        #            #print(torch.sum(torch.abs(M_inter)))
        #            loss_W += torch.sum(torch.abs(M_inter))
        #print(loss_W, ms.loss)
        #loss_W = loss_W/(100*self._net.features[15].featuresf.shape[0]^2)
        #print(loss_W, ms.loss)
        #ms.loss += 0.00005*loss_W
...
def main():
    ...
    parser.add_argument("--nchannel", type=int)
    ...
    net = net_class.create_with_load(
        fpath=args.load,
        kwargs={'args': args, 'quant_step': args.input_quantization},
